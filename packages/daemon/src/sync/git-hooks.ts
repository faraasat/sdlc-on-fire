...
function hookScript(command: string): string {
  return [
    ...
    '#',
    '# The last branch is quiet on purpose. Before the package is installed',
    '# anywhere, npx prints a registry error, and a user should not see a wall of',
    '# npm output on every single commit for a sync they never asked for.',
    'if command -v sdlc >/dev/null 2>&1; then',
    `  ${command} || true`,
    'elif [ -x ./node_modules/.bin/sdlc ]; then',
    `  ./node_modules/.bin/${command} || true`,
    'else',
    `  npx --no-install ${command.replace(/^sdlc\b/, 'sdlc-on-fire')} >/dev/null 2>&1 || true`,
    'fi',
    MARKER.replace('>>>', '<<<'),
    '',
  ].join('\n');
